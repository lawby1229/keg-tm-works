...
public class AnalyseMobileVersionFeature {
				...
				if (features.length > 1) {
					...
					for (int i = 3; i < features.length; i++) {
						...
						if (!Dimensions.containsKey(feature[0].trim())) {
							DimensionsArray.add(feature[0].trim());
	...
	public void outputFile(String fileName) {
		try {
			...
			// }
			fw.write(",");
			for (int j = 1; j < DimensionsArray.size(); j++) {

				fw.write(DimensionsArray.get(j) + ",");
			...
			for (int i = 0; i < FeaturesAllLine.size(); i++) {
				...
				for (int j = 1; j <= Dimensions.size(); j++) {
					if (lineFeature.containsKey(j))
						fw.write(lineFeature.get(j) + ",");
					else
						fw.write("" + ",");
				}
				fw.write("\b\n");
